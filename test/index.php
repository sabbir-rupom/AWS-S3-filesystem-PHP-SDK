 ...
 */
$tempUploadPath = create_directory(__DIR__ . DIRECTORY_SEPARATOR . 'temp-aws');
    ...
    for ($i = 0; $i < $total; $i++) {
        ...
        if ($tmpFilePath != "") { // Make sure file path is not empty
            ...
            if (move_uploaded_file($tmpFilePath, $fileSource)) {
                ...
                } else if ($uploadResutl['success'] == FALSE) {
    ...
    <body>
        ...
        if (isset($dirName) && !empty($dirName) && isset($bucketName) && !empty($bucketName)) {
            ...
            if (count($result['fileList']) > 0) {
                ...
                ?>
                Following files are uploaded in <?= $dirName; ?> directory:<br>
                <ul>
                    <?php
                    foreach ($files as $k => $v) {
                        ...
                        [ <a href="<?= $awsSdkObj->getS3UrlPath($bucketName) . $filePath; ?>" target="_BLANK">S3 Link</a> ] 
                        [ <a href="?delete=1&directory=<?= urlencode($dirName); ?>&file_key=<?= urlencode($filePath); ?>&bucket=<?= urlencode($bucketName); ?>">Delete</a> ] 
                        <?php
                        echo '</li>';
                    }
                    ?>
                ...
                <br>
                <a href="?delete=2&directory=<?= urlencode($dirName); ?>&bucket=<?= urlencode($bucketName); ?>">Delete Directory?</a>
                <?php
            }
        }
        ?>
